...
void merge(tree_node** left_node_list, tree_node** right_node_list, int left_list_length, int right_list_length)
{
	...
	//Sorts list items first by species, then by seed, then by age
	while(left_list_counter < left_list_length || right_list_counter < right_list_length)
	{
		...
		{//If left and right list item have the same species
			...
			{//If left and right list item have the same seed
	...
	//Copy temp list back into left list
...
//Finds first model cache which is not referenced
//meaning that the model cache doesn't contain valid model data
...
//Add reference mapping the given tree_node to:
//	- Unused model cache (if model_index = -1)
//	- Model cache with index model_index in the model cache array
void tree_model_map::add_model_ref(tree_node* t_node, int model_index)
{
	for(int i = 0; i < number_of_tree_model_refs; i++)
	{
		...
		{//If current tree_model_ref is not being used
			tree_model_refs[i].tree = t_node;
			tree_model_refs[i].model_ref = (model_index >= 0) ? model_index : find_available_model_cache();
			tree_models[tree_model_refs[i].model_ref].ref_count++; //Increase reference count in corresponding model_cache for t_node
			break;
		}
	}
}

//Find tree_model_ref which contains t_node as its owner
